...
⚠️ **Важно:** В тестовом окружении включена автосинхронизация БД (`alter: true`). 
При перезапуске сервера Sequelize автоматически добавит недостающие колонки в таблицы.

В продакшене (`NODE_ENV=production`) автосинхронизация отключена для безопасности.
...
# В логах должно быть: "✅ Модели синхронизированы с базой данных."
# Если были добавлены колонки, вы увидите соответствующие SQL запросы
```
